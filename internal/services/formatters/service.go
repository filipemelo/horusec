...
func (s *Service) RemoveSrcFolderFromPath(pathFromFile string) string {
	if pathFromFile == "" || len(pathFromFile) <= 4 || !strings.Contains(pathFromFile[:4], "src") {
		return pathFromFile
	}

	return pathFromFile[5:]
}
...
func (s *Service) GetFilepathFromFilename(filename, projectSubPath string) string {
	...
	return path.Join(projectSubPath, filepathWithFileName)
}
...
//TODO: why this function is returning error?
...
func (s *Service) removeHorusecFolder(pathFromFile string) string {
	toRemove := fmt.Sprintf("%s/", s.GetConfigProjectPath())
	return strings.ReplaceAll(pathFromFile, toRemove, "")
}
